/**
 * gen-day11-tests.js — Day 11: Notification System Test Generator (CORRECTED)
 * Run: node gen-day11-tests.js
 *
 * Uses the ACTUAL project structure:
 *   - authMiddleware (not auth.js)
 *   - rbacMiddleware (not authorize from auth.js)
 *   - Custom validation (not Joi)
 *   - SmsService.sendSMS() (existing service)
 *   - Personnel login via CIN + password
 */
...
beforeAll(async () => {
  ...
  parentToken = verifyRes.body.data?.token || verifyRes.body.token;
  parentId = verifyRes.body.data?.user?.id || verifyRes.body.user?.id;
  ...
  const nurseLogin = await request(app)
    ...
    .send({ cin: 'IN1', mot_de_passe: 'password123' });

  nurseToken = nurseLogin.body.data?.token || nurseLogin.body.token;
  nurseId = nurseLogin.body.data?.user?.id || nurseLogin.body.user?.id;

  // Login as admin
  const adminLogin = await request(app)
    .post('/api/auth/personnel/login')
    .send({ cin: 'AD1', mot_de_passe: 'password123' });
...
console.log('\n🚀 Generating Day 11 Tests (corrected)…\n');
writeFile('tests/notifications.test.js', testFile);
console.log('\n✨ Day 11 test generation complete!\n');
