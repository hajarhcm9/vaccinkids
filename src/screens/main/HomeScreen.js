...
const HomeScreen = ({ navigation }) => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setBabies(babiesRes);
      const delayedByBaby = await Promise.all(
        babiesRes.map(async (baby) => {
          const delayed = await healthBookService.getDelayed(baby.id).catch(() => []);
          return delayed.map((vaccine) => ({
            id: `${baby.id}-${vaccine.vaccin_nom}-${vaccine.age_cible_semaines}`,
            baby: baby.firstName,
            vaccine: vaccine.vaccin_nom,
            daysLate: Math.max(
              1,
              Math.round(
                (Number(vaccine.age_actuel_semaines) - Number(vaccine.age_cible_semaines)) * 7,
              ),
            ),
          }));
        }),
        ...
        {babies.length === 0 ? (
          ...
          babies.map((baby) => (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('HealthBook', { babyId: baby.id })}
            >
              <View style={styles.babyAvatar}>
                <Text style={styles.babyAvatarEmoji}>{baby.gender === 'male' ? '👦' : '👧'}</Text>
              </View>
        ...
        <View style={styles.educBanner}>
          ...
          <View style={styles.educContent}>
            ...
            <Text style={styles.educText}>
              Les vaccins protègent votre enfant contre des maladies graves dès les premiers mois de
              vie.
            </Text>
          </View>
        </View>
