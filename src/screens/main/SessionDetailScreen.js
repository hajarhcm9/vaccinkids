...
const SessionDetailScreen = ({ route, navigation }) => {
  ...
  const MOCK_BABIES = [
    { id: 'baby_1', name: 'Youssef Benali' },
    { id: 'baby_2', name: 'Nour Benali' },
  ];
  const [selectedBaby, setSelectedBaby] = useState(MOCK_BABIES[0]);
  ...
  const handleBook = async () => {
    ...
    try {
      const res = await sessionService.bookSession(sessionId, selectedBaby.id, selectedBaby.name);
      setBooking(res.booking);
      setFillData((prev) => ({ ...prev, bookedSlots: prev.bookedSlots + 1 }));
      Alert.alert(
        '✅ Réservation confirmée !',
        `Place réservée pour ${selectedBaby.name}.\nNuméro de file : ${res.booking.queueNumber}`
      );
    } catch (err) {
      Alert.alert('Erreur', err.message);
    } finally {
  ...
  const handleWaitlist = async () => {
    setActionLoading(true);
    try {
      const res = await sessionService.joinWaitlist(sessionId, selectedBaby.id, selectedBaby.name);
      setBooking(res.booking);
      Alert.alert(
        "⏳ Liste d'attente",
        `Vous êtes en position ${res.booking.waitlistPosition} pour ${selectedBaby.name}.`
      );
    } catch (err) {
      Alert.alert('Erreur', err.message);
    } finally {
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
          ...
          {[
            ...
            { icon: '👨‍⚕️', label: 'Médecin', value: session.doctorName },
            ...
            {MOCK_BABIES.map((baby) => (
              ...
              >
                ...
                {selectedBaby.id === baby.id && (
                  <Text style={styles.babyOptionCheck}>✓</Text>
                )}
              </TouchableOpacity>
            ))}
          </View>
...
const styles = StyleSheet.create({
  ...
  backBtn: { width: 80 },
  backText: { color: 'rgba(255,255,255,0.85)', fontSize: typography.fontSizes.md, fontWeight: typography.fontWeights.medium },
  headerTitle: { fontSize: typography.fontSizes.lg, fontWeight: typography.fontWeights.bold, color: colors.white },
  scroll: { flex: 1 },
  scrollContent: { padding: spacing.lg, paddingBottom: spacing.xxxl, gap: spacing.md },
  bookingBanner: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, borderRadius: borderRadius.md, padding: spacing.lg, borderLeftWidth: 4 },
  ...
  bookingBannerTitle: { fontSize: typography.fontSizes.sm, fontWeight: typography.fontWeights.bold, color: colors.textPrimary },
  bookingBannerSub: { fontSize: typography.fontSizes.xs, color: colors.textSecondary, marginTop: 2 },
  card: { backgroundColor: colors.surface, borderRadius: borderRadius.lg, padding: spacing.lg, ...shadows.card, gap: spacing.md },
  vaccineTitle: { fontSize: typography.fontSizes.xl, fontWeight: typography.fontWeights.bold, color: colors.textPrimary, marginBottom: spacing.xs },
  ...
  liveText: { fontSize: typography.fontSizes.xs, fontWeight: typography.fontWeights.bold, color: colors.success, letterSpacing: 0.5 },
  babyOption: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.md, borderRadius: borderRadius.md, borderWidth: 1.5, borderColor: colors.border, backgroundColor: colors.inputBg },
  ...
  actionsCard: { backgroundColor: colors.surface, borderRadius: borderRadius.lg, padding: spacing.lg, ...shadows.card, gap: spacing.sm },
  btnPrimary: { backgroundColor: colors.primary, borderRadius: borderRadius.md, height: 52, justifyContent: 'center', alignItems: 'center', ...shadows.button },
  btnPrimaryText: { color: colors.white, fontSize: typography.fontSizes.md, fontWeight: typography.fontWeights.semibold },
  btnWaitlist: { backgroundColor: colors.warningLight, borderRadius: borderRadius.md, height: 52, justifyContent: 'center', alignItems: 'center', borderWidth: 1.5, borderColor: colors.warning },
  btnWaitlistText: { color: colors.warning, fontSize: typography.fontSizes.md, fontWeight: typography.fontWeights.semibold },
  btnCancel: { backgroundColor: colors.dangerLight, borderRadius: borderRadius.md, height: 52, justifyContent: 'center', alignItems: 'center', borderWidth: 1.5, borderColor: colors.danger },
  btnCancelText: { color: colors.danger, fontSize: typography.fontSizes.md, fontWeight: typography.fontWeights.semibold },
