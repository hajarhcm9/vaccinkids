...
const AppointmentsScreen = ({ navigation }) => {
  ...
  const handleCancel = (booking) => {
    Alert.alert('Annuler', `Annuler la réservation pour ${booking.babyName} ?`, [
  ...
  const renderBooking = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          session.id && navigation.navigate('SessionDetail', { sessionId: session.id })
        }
        activeOpacity={0.8}
      >
        ...
        <View style={styles.cardBody}>
          ...
          {item.status === 'confirmed' && item.queueNumber && (
      ...
      {sections.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtitle}>
            Réservez une session pour commencer le suivi vaccinal.
          </Text>
