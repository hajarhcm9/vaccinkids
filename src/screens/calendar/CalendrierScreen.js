...
const MOCK_VACCINS = {
  '1': [
    { id: 'v1', nom: 'BCG',          date_prevue: 'Naissance',  statut: 'FAIT' },
    { id: 'v2', nom: 'Hépatite B 1', date_prevue: '2 mois',     statut: 'FAIT' },
    { id: 'v3', nom: 'DTP 1',        date_prevue: '2 mois',     statut: 'FAIT' },
    { id: 'v4', nom: 'DTP 2',        date_prevue: '4 mois',     statut: 'EN_RETARD' },
    { id: 'v5', nom: 'Hépatite B 2', date_prevue: '4 mois',     statut: 'A_VENIR' },
    { id: 'v6', nom: 'Rougeole 1',   date_prevue: '12 mois',    statut: 'A_VENIR' },
  ],
  '2': [
    { id: 'v7', nom: 'BCG',          date_prevue: 'Naissance',  statut: 'FAIT' },
    { id: 'v8', nom: 'Hépatite B 1', date_prevue: '2 mois',     statut: 'FAIT' },
    { id: 'v9', nom: 'DTP 1',        date_prevue: '2 mois',     statut: 'FAIT' },
    { id: 'v10', nom: 'DTP 2',       date_prevue: '4 mois',     statut: 'FAIT' },
    { id: 'v11', nom: 'Hépatite B 2',date_prevue: '4 mois',     statut: 'A_VENIR' },
    { id: 'v12', nom: 'Rougeole 1',  date_prevue: '12 mois',    statut: 'A_VENIR' },
    { id: 'v13', nom: 'Varicelle',   date_prevue: '18 mois',    statut: 'A_VENIR' },
  ],
};
...
export default function CalendrierScreen({ route, navigation }) {
  ...
  const [selectedEnfant, setSelectedEnfant] = useState(route?.params?.enfantId || MOCK_ENFANTS[0].id);
  const [vaccins, setVaccins] = useState(MOCK_VACCINS[MOCK_ENFANTS[0].id] || []);

  React.useEffect(() => {
    enfantService.listEnfants().then((data) => {
      if (Array.isArray(data) && data.length > 0) {
        setEnfants(data);
        if (!route?.params?.enfantId) setSelectedEnfant(data[0].id);
      }
    }).catch(() => {});
  }, []);
  ...
  React.useEffect(() => {
    ...
    vaccinService.getCalendrierEnfant(selectedEnfant)
      .then((data) => { if (Array.isArray(data) && data.length > 0) setVaccins(data); })
      .catch(() => { setVaccins(MOCK_VACCINS[selectedEnfant] || MOCK_VACCINS['1']); });
  }, [selectedEnfant]);

  const handleSelectEnfant = (id) => {
    setSelectedEnfant(id);
    setVaccins(MOCK_VACCINS[id] || MOCK_VACCINS['1']);
  };
  ...
  const renderVaccin = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.row, item.statut === 'EN_RETARD' && styles.rowAlert]}
        activeOpacity={0.8}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {enfants.map((enfant) => (
            <TouchableOpacity
              ...
              onPress={() => handleSelectEnfant(enfant.id)}
              accessibilityRole="tab"
              accessibilityState={{ selected: selectedEnfant === enfant.id }}
            >
              <Text style={[styles.enfantPillText, selectedEnfant === enfant.id && styles.enfantPillTextActive]}>
                {enfant.prenom}
              </Text>
            </TouchableOpacity>
          ))}
