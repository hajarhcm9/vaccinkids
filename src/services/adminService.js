...
class AdminService {
  ...
  async createPersonnel(data, adminId) {
    ...
    // Validate centre
    var centreRes = await pool.query('SELECT id FROM centre WHERE id = $1 AND est_actif = TRUE', [centre_id]);
    ...
    try {
      await pool.query(
        ...
        ['personnel', personnel.id, 'INSERT', JSON.stringify({ cin: cin, nom: nom, prenom: prenom, role: role, centre_id: centre_id }), adminId, 'admin']
      );
  ...
  async getPersonnel(filters) {
    ...
    if (search) {
      conditions.push('(p.nom ILIKE $' + idx + ' OR p.prenom ILIKE $' + idx + ' OR p.cin ILIKE $' + idx + ')');
      params.push('%' + search + '%');
      idx++;
    }
    ...
    var dataSQL = 'SELECT p.id, p.cin, p.nom, p.prenom, p.role, p.centre_id, p.est_actif, p.created_at, p.updated_at, c.nom AS centre_nom'
      + ' FROM personnel p'
  ...
  async getPersonnelById(id) {
    var sql = 'SELECT p.id, p.cin, p.nom, p.prenom, p.role, p.centre_id, p.est_actif, p.created_at, p.updated_at, c.nom AS centre_nom, c.adresse AS centre_adresse'
      + ' FROM personnel p'
      + ' LEFT JOIN centre c ON c.id = p.centre_id'
      + ' WHERE p.id = $1';
  ...
  async updatePersonnel(id, data, adminId) {
    ...
    for (var i = 0; i < allowedFields.length; i++) {
      ...
      if (data[field] !== undefined) {
        ...
        if (field === 'centre_id') {
          var cRes = await pool.query('SELECT id FROM centre WHERE id = $1 AND est_actif = TRUE', [data[field]]);
    ...
    var sql = 'UPDATE personnel SET ' + updates.join(', ') + ' WHERE id = $' + idx
    ...
    try {
      await pool.query(
        ...
        ['personnel', id, 'UPDATE', JSON.stringify({ nom: old.nom, role: old.role, est_actif: old.est_actif }), JSON.stringify(data), adminId, 'admin']
      );
    } catch (e) { /* non-blocking */ }
  ...
  async deactivatePersonnel(id, adminId) {
    ...
    try {
      await pool.query(
        'INSERT INTO audit_log (table_name, record_id, action, old_values, new_values, user_id, user_role) VALUES ($1, $2, $3, $4, $5, $6)',
        ['personnel', id, 'UPDATE', JSON.stringify({ est_actif: true }), JSON.stringify({ est_actif: false }), adminId, 'admin']
      );
    } catch (e) { /* non-blocking */ }
  ...
  async reactivatePersonnel(id, adminId) {
    ...
    try {
      await pool.query(
        'INSERT INTO audit_log (table_name, record_id, action, old_values, new_values, user_id, user_role) VALUES ($1, $2, $3, $4, $5, $6)',
        ['personnel', id, 'UPDATE', JSON.stringify({ est_actif: false }), JSON.stringify({ est_actif: true }), adminId, 'admin']
      );
    } catch (e) { /* non-blocking */ }
  ...
  async createCentre(data, adminId) {
    ...
    try {
      await pool.query(
        'INSERT INTO audit_log (table_name, record_id, action, new_values, user_id, user_role) VALUES ($1, $2, $3, $4, $5, $6)',
        ['centre', centre.id, 'INSERT', JSON.stringify({ nom: nom, adresse: adresse, telephone: telephone }), adminId, 'admin']
      );
    } catch (e) { /* non-blocking */ }
  ...
  async getCentres(filters) {
    ...
    var dataSQL = 'SELECT c.id, c.nom, c.adresse, c.telephone, c.gps_lat, c.gps_lng, c.est_actif, c.created_at, c.updated_at,'
  ...
  async getCentreById(id) {
    var sql = 'SELECT c.id, c.nom, c.adresse, c.telephone, c.gps_lat, c.gps_lng, c.est_actif, c.created_at, c.updated_at,'
      + ' (SELECT COUNT(*) FROM personnel p WHERE p.centre_id = c.id AND p.est_actif = TRUE) AS nb_personnel,'
      + ' (SELECT COUNT(*) FROM session s WHERE s.centre_id = c.id) AS nb_sessions,'
  ...
  async updateCentre(id, data, adminId) {
    ...
    var sql = 'UPDATE centre SET ' + updates.join(', ') + ' WHERE id = $' + idx
    ...
    try {
      await pool.query(
        'INSERT INTO audit_log (table_name, record_id, action, old_values, new_values, user_id, user_role) VALUES ($1, $2, $3, $4, $5, $6)',
        ['centre', id, 'UPDATE', JSON.stringify({ nom: old.nom, adresse: old.adresse }), JSON.stringify(data), adminId, 'admin']
      );
    } catch (e) { /* non-blocking */ }
  ...
  async deactivateCentre(id, adminId) {
    ...
    if (parseInt(personnelRes.rows[0].total) > 0) {
      return { error: "Impossible de desactiver un centre avec du personnel actif. Desactivez le personnel d'abord." };
    }
    ...
    try {
      await pool.query(
        'INSERT INTO audit_log (table_name, record_id, action, old_values, new_values, user_id, user_role) VALUES ($1, $2, $3, $4, $5, $6)',
        ['centre', id, 'UPDATE', JSON.stringify({ est_actif: true }), JSON.stringify({ est_actif: false }), adminId, 'admin']
      );
    } catch (e) { /* non-blocking */ }
  ...
  async getAuditLog(filters) {
    ...
    if (search) {
      conditions.push('(a.old_values::text ILIKE $' + idx + ' OR a.new_values::text ILIKE $' + idx + ')');
      params.push('%' + search + '%');
      idx++;
    }
    ...
    // Data
    var dataSQL = 'SELECT a.id, a.table_name, a.record_id, a.action, a.old_values, a.new_values, a.user_id, a.user_role, a.timestamp'
      + ' FROM audit_log a'
      + whereClause
  ...
  async getAuditStats() {
    ...
    var actionRes = await pool.query('SELECT action, COUNT(*) AS nb FROM audit_log GROUP BY action ORDER BY nb DESC');
    var tableRes = await pool.query('SELECT table_name, COUNT(*) AS nb FROM audit_log GROUP BY table_name ORDER BY nb DESC');
    var recentRes = await pool.query("SELECT COUNT(*) AS nb FROM audit_log WHERE timestamp >= NOW() - INTERVAL '24 hours'");
    var dailyRes = await pool.query("SELECT DATE(timestamp) AS jour, COUNT(*) AS nb FROM audit_log WHERE timestamp >= NOW() - INTERVAL '7 days' GROUP BY DATE(timestamp) ORDER BY jour DESC");
  ...
  async getSystemInfo() {
    ...
    var activePersonnel = await pool.query('SELECT COUNT(*) AS total FROM personnel WHERE est_actif = TRUE');
