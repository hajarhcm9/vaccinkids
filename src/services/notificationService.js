...
class NotificationService {
  async sendNotification(o) {
    ...
    if (sent) {
      ...
      n.envoye = true; n.date_envoi = new Date();
    }
    return n;
  }
  async _gp(id) {
  ...
  _ds(si) {
    return si ? new Date(si.date_session+"T"+si.heure_debut).toLocaleString("fr-FR",{dateStyle:"long",timeStyle:"short"}) : "votre prochaine session";
  }
  async sendRappelRdv(rv) {
    const phone = await this._gp(rv.parent_id);
    const ds = this._ds(await this._gs(rv.session_id));
    return this.sendNotification({destinataire_id: rv.parent_id, destinataire_type: "parent", type: "RAPPEL_RDV", canal: "sms", titre: "Rappel de rendez-vous", message: "Rappel: vous avez un RDV le "+ds+".", reference_id: rv.id, reference_type: "rendez_vous", phone});
  }
  async sendAbsenceNotification(rv) {
    const phone = await this._gp(rv.parent_id);
    const ds = this._ds(await this._gs(rv.session_id));
    return this.sendNotification({destinataire_id: rv.parent_id, destinataire_type: "parent", type: "ABSENCE", canal: "sms", titre: "Absence au rendez-vous", message: "Votre RDV du "+ds+" a ete marque absent.", reference_id: rv.id, reference_type: "rendez_vous", phone});
  }
